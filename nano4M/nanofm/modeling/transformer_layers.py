...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        qkv = self.qkv_proj(x)  # shape: [B, L, 3 * D]
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        kv = self.kv_proj(context).view(B, M, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        x_a = x + self.attn(self.norm1(x), mask=mask)
        x_b = x_a + self.mlp(self.norm2(x_a))
        return x_b
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, 
        ...
        x = x + self.self_attn(self.norm1(x), mask=sa_mask) # TODO
