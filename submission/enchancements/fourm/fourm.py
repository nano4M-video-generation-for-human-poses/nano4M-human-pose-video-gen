...
class FourM(nn.Module):
    ...
    def forward_encoder(
        ...
        if enc_pad_mask is None:
            enc_pad_mask = torch.ones(enc_input_tokens.shape, dtype=torch.bool, device=enc_input_tokens.device)

        # Construct (B, N, N) attention mask for padding. True = used, False = masked out.
        enc_pad_attn_mask = repeat(enc_pad_mask, 'b n -> b m n', m=N)
    ...
    def forward_decoder(
        ...
        # ----- 1. derive “valid” masks -----------------------------------------
        valid_q   = (dec_pad_mask if dec_pad_mask is not None else torch.ones(B, M, device=x.device)).bool()
        valid_enc = (enc_pad_mask if enc_pad_mask is not None else torch.ones(B, N, device=x.device)).bool()
        
        # ----- 2. build attention masks ----------------------------------------
        dec_pad_sa_mask = valid_q.unsqueeze(2) & valid_q.unsqueeze(1)       # (B, M, M)
        dec_pad_xa_mask = valid_q.unsqueeze(2) & valid_enc.unsqueeze(1)     # (B, M, N)
        
        # ----- 3. guarantee each query sees ≥1 key -----------------------------
        row_ok_sa = dec_pad_sa_mask.any(-1, keepdim=True)
        row_ok_xa = dec_pad_xa_mask.any(-1, keepdim=True)
        dec_pad_sa_mask = torch.where(row_ok_sa, dec_pad_sa_mask, torch.ones_like(dec_pad_sa_mask))
        dec_pad_xa_mask = torch.where(row_ok_xa, dec_pad_xa_mask, torch.ones_like(dec_pad_xa_mask))
        
        # ----- 4. zero embeddings of padded queries ----------------------------
        x = x.masked_fill(~valid_q.unsqueeze(-1), 0.)
    ...
    def compute_ce_loss(
        ...
        # --------------------------------------------------------------------
        # Watchdog 1 – logits must be finite
        # --------------------------------------------------------------------
        bad_mask = ~torch.isfinite(logits)               # bool tensor shaped like logits
        if bad_mask.any():
            bad_flat  = bad_mask.view(-1).nonzero(as_tuple=False)   # (N_bad, 1)
            first_bad = bad_flat[0, 0].item()
            print(f"[NaN logits] first bad index = {first_bad}  "
                  f"min={logits.min().item():.6g}  max={logits.max().item():.6g}")
            raise RuntimeError("non-finite logits – aborting step")
        # --------------------------------------------------------------------
        # Watchdog 2 – target ids inside [0, vocab_size-1] or == padding_idx
        # --------------------------------------------------------------------
        ok_id = (target_seq.eq(padding_idx) |
                 ((target_seq >= 0) & (target_seq < vocab_size)))
        if not ok_id.all():
            bad_ids = target_seq[~ok_id][:16].tolist()
            print(f"[Bad target id] {bad_ids}  (vocab size = {vocab_size})")
            raise RuntimeError("target ids out of range")
    
        # (optional) Watchdog 3 – parameters that turned non-finite
        # Comment this out once training is stable; looping every step is slow.
        for n, p in self.named_parameters():
            if p.requires_grad and (not torch.isfinite(p).all()):
                print(f"[Bad param] {n}  min={p.min().item():.6g}  "
                      f"max={p.max().item():.6g}")
        ...
        if not per_modality_loss_avg:
            ...
            losses = F.cross_entropy(logits, target_seq, reduction='none', ignore_index=padding_idx)
    ...
    def generate_one_modality_roar(
        ...
        rand_indices = torch.randperm(n_tokens_target, device=device)
        ...
        dec_input_positions_list = [rand_indices[start:start+k].unsqueeze(0) for start, k in zip(torch.cumsum(torch.tensor([0] + schedule[:-1]), dim=0), schedule)]        
        for step, k in enumerate(schedule):
            ...
            logits = self.forward_model(enc_input_tokens,enc_input_modalities,enc_input_positions,dec_input_modalities,dec_input_positions)
