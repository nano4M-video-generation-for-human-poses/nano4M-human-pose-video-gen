    ...
    def __getitem__(self, idx):
        ...
        for m in self.modalities:
            ...
            if "tok_rgb" == m:
                ...
                kk  = k if (arr.ndim == 3 and k < arr.shape[0]) else 0
                frame = arr[kk] if arr.ndim == 3 else arr      # (3,13)

                # ----- 1) x-,y-coordinates → 1 … 8190 ----------
                xy      = frame[:2]                             # (2,13) float32
                ...
                # ----- 3) flatten to 39-token sequence --------
                tokens  = np.concatenate([xy_tok.flatten(), vis_tok.flatten()])
