...
NEXT_LEN           = 6                # “_next” modalities contain 6 frames
    ...
    def __getitem__(self, idx: int):
        ...
        for m in self.modalities:
            ...
            # -------------------------------------------------- coords --------
                ...
                path = next(fdir.glob(f"*{ext}"))
                arr  = np.load(path, mmap_mode="r", allow_pickle=True)   # (K,3,13) or (3,13)
            
                if self._looks_like_raw_xy(arr):                         # real floats
                    frame  = arr[k] if arr.ndim == 3 else arr            # (3,13)
                    tok    = self._quantise_coords(frame)                # (39,)
                else:                                                    # already tokens
                    tok = arr[k] if arr.ndim == 3 else arr               # (3,13) ints
                    tok = tok.reshape(-1).astype(np.int64)               # (39,)
            
                sample[m] = torch.from_numpy(tok)
    
            # ------------------------------------------- coords_next ----------
            elif m == "coords_next":
                frame_tokens = []
                for p in sorted(fdir.glob(f"*{ext}")):                   # 6 files
                    arr = np.load(p, mmap_mode="r", allow_pickle=True)   # (K,3,13)/(3,13)/(K,39)/(39,)
            
                    if self._looks_like_raw_xy(arr):                     # raw floats
                        frame = arr[k] if arr.ndim == 3 else arr         # (3,13)
                        tok   = self._quantise_coords(frame)
                    else:                                                # already tokens
                        tok = arr[k] if arr.ndim == 3 else arr           # (39,)
                        tok = tok.astype(np.int64)
            
                    frame_tokens.append(tok)
            
                sample[m] = torch.from_numpy(np.concatenate(frame_tokens))  # (234,)
